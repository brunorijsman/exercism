...
var transformPanicTests = []struct {
	input given
}{
	{
		given{1: {"ABCD"}},
...
func TestTransform(t *testing.T) {
	...
	for _, tpt := range transformPanicTests {
		defer func() { recover() }()
		actual := Transform(map[int][]string(tpt.input))
		t.Fatalf("Transform(%v). Expected panic, Actual [%v]", tpt.input, actual)
	}
}
